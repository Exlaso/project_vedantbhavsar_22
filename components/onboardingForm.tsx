...
export function OnboardingForm({questions,currentIDX}: { questions: Tables<'general_questions'>[],currentIDX:number }) {
    const router = useRouter();
    const {data: currentIndex, refetch} = useQuery({
        queryKey: ["Current Index"],
        queryFn: getCurrentProgress,
        enabled:false,
        initialData: () => {return currentIDX}
    })
    ...
    const question = questions.at(currentIndex || 0)
    if (!question) {
        return <div>
